  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
        ...
        <p className="text-xs text-muted-foreground mb-3">Max returnable: <span className="text-amber-500 font-semibold">{maxReturn}</span> units</p>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleConfirm} disabled={qty <= 0 || qty > maxReturn}
            className="flex-1 px-4 py-2.5 rounded-lg bg-amber-600 hover:bg-amber-700 text-white font-semibold transition-colors text-sm disabled:opacity-40 disabled:cursor-not-allowed">Return {qty}</button>
        </div>
      </div>
    </div>
  )
