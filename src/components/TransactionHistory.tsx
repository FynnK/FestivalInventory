      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" className="w-full bg-background border border-input rounded-lg py-2 pl-9 pr-3 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
            placeholder="Search by item, barcode, stage, note..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        ...
        <span className="text-xs text-muted-foreground whitespace-nowrap">{filtered.length} of {ledger.length} transactions</span>
      </div>

      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {filtered.length === 0 ? (
          <div className="flex items-center justify-center h-48 text-muted-foreground text-lg font-medium">No transactions found</div>
        ) : (
              ...
              {filtered.map(entry => (
                <tr key={entry.id} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                  ...
                  <td className="py-2.5 px-3">
                    {entry.type === 'issue' ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-emerald-600/20 text-emerald-600 dark:text-emerald-400"><ArrowUpRight size={12} /> Issue</span>
                    ) : entry.type === 'return' ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-amber-600/20 text-amber-600 dark:text-amber-400"><ArrowDownLeft size={12} /> Return</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold bg-blue-600/20 text-blue-600 dark:text-blue-400"><AlertTriangle size={12} /> Adj.</span>
                    )}
                  </td>
                  ...
                  <td className="py-2.5 px-3 text-right">
                    <button onClick={() => { setReverseEntry(entry); setReverseNote('') }} className="p-1.5 text-muted-foreground hover:text-primary transition-colors" title="Reverse this transaction"><RotateCcw size={14} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-md mx-4 shadow-2xl">
            ...
            <p className="text-muted-foreground text-sm mb-3">
              ...
              <strong>{reverseEntry.qtyIssued || reverseEntry.qtyReturned}x {reverseEntry.itemName}</strong> at <strong>{reverseEntry.stageName}</strong>
            </p>
            <div className="mb-4">
              <label className="block text-xs font-medium text-muted-foreground mb-1">Reason / Comment</label>
              <textarea value={reverseNote} onChange={e => setReverseNote(e.target.value)} placeholder="Why are you reversing this?"
            ...
            <div className="flex gap-3">
              <button onClick={() => setReverseEntry(null)} className="flex-1 px-4 py-2 rounded-lg border border-input text-foreground hover:bg-accent transition-colors font-medium">Cancel</button>
              <button onClick={() => { onReverse(reverseEntry.id, reverseNote); setReverseEntry(null) }}
                className="flex-1 px-4 py-2 rounded-lg bg-destructive text-destructive-foreground font-semibold hover:opacity-90 transition-opacity">Reverse</button>
            </div>
          </div>
        </div>
      )}
