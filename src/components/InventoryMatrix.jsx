      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {filtered.length === 0 ? (
          <div className="flex items-center justify-center h-48 text-muted-foreground text-lg font-medium">
            {search || categoryFilter !== 'all' ? 'No items match your filters' : 'No items in inventory'}
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-background z-10">
              <tr className="border-b border-border">
                <th className="text-left py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Barcode</th>
                <th className="text-left py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Name</th>
                <th className="text-left py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Category</th>
                <th className="text-center py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Stock</th>
                <th className="text-center py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Status</th>
                {activeStageId && (
                  <th className="text-center py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">At Stage</th>
                )}
                <th className="text-right py-2.5 px-2 font-semibold text-muted-foreground text-xs uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(item => {
                const netAtStage = activeStageId
                  ? item.stageBreakdown?.[activeStageId] || 0
                  : 0
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-2.5 px-2 text-center">
                      <span className="font-bold text-lg text-foreground">{Math.max(0, item.remaining)}</span>
                      <span className="text-xs text-muted-foreground ml-1">/ {item.total}</span>
                      {item.unitType && (
                        <div className="text-xs text-muted-foreground">
                          {item.unitQuantity > 1 ? `${item.unitQuantity} ${item.unitType}` : item.unitType}
                        </div>
                      )}
                    </td>
                    <td className="py-2.5 px-2 text-center">
