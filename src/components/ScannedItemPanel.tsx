  ...
  return (
    <div className="bg-card border border-border rounded-lg p-3">
      <div className="flex items-center justify-between mb-2">
        ...
        {totalItems > 0 && <span className="text-xs text-muted-foreground">{uniqueItems} items ({totalItems} total)</span>}
      </div>
      {totalItems === 0 ? (
        <p className="text-sm text-muted-foreground text-center py-4">No items scanned yet.<br />Scan barcodes to build your cart.</p>
      ) : (
        <>
            ...
            {cart.map((entry, idx) => (
              <div key={`${entry.item.id}-${idx}`} className="flex items-center gap-1.5 bg-accent/30 rounded-lg p-2">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => onAddQty(entry.item)} className="w-6 h-6 rounded bg-accent hover:bg-accent/80 text-foreground flex items-center justify-center transition-colors">
                    <Plus size={12} />
                  </button>
                  <button onClick={() => onRemove(entry.item.id)} className="w-6 h-6 rounded hover:bg-red-500/20 text-muted-foreground hover:text-red-500 flex items-center justify-center transition-colors">
                    <X size={12} />
                  </button>
                </div>
              </div>
          ...
          <button onClick={onCheckout} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 transition-opacity">
            <ShoppingCart size={16} />Checkout{stageName ? ` to ${stageName}` : ''}
          </button>
          {!stageName && <p className="text-xs text-amber-500 mt-1.5 text-center">Select a stage or choose one during checkout.</p>}
        </>
      )}
    </div>
  )
