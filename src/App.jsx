...
import {
  ScanLine, Plus, XCircle, CheckCircle, AlertTriangle,
  ArrowLeftRight, Download, Upload, Tent, Warehouse,
  ListChecks, BarChart3, History,
} from 'lucide-react'
...
import {
  ...
  getNetIssuedToStage, getLedger, getLedgerWithDetails,
  reverseTransaction,
...
export default function App() {
  ...
  const tryAddToCart = useCallback((item) => {
    ...
    setCart(prev => {
      ...
      if (desiredQty > item.remaining) {
        setAddStockPrompt({ item, desiredQty, currentQty })
        return prev
      }
      if (existing) {
        return prev.map(c =>
          c.item.id === item.id ? { ...c, qty: desiredQty } : c
        )
      }
  ...
  const handleConfirmAddStock = () => {
    ...
    const { item, desiredQty, currentQty } = addStockPrompt
    const freshItem = getItemById(item.id) || item
    const shortfall = desiredQty - freshItem.remaining
    if (shortfall > 0) {
      updateItem(item.id, { total: freshItem.total + shortfall })
    }
    setCart(prev => {
      const existing = prev.find(c => c.item.id === item.id)
      if (existing) {
      ...
      return [...prev, { item: { ...freshItem, total: freshItem.total + Math.max(0, shortfall), remaining: freshItem.remaining + Math.max(0, shortfall) }, qty: desiredQty }]
    })
  ...
  const handleCheckout = () => {
    ...
    if (activeStageId) {
      setCheckoutReview({ stageId: activeStageId, cart: [...cart] })
    } else {
      setCheckoutStagePicker(true)
    }
  }

  const handleCheckoutStagePick = (stageId) => {
  ...
  const confirmCheckout = () => {
    ...
    if (successCount > 0) {
      showToast(`Checked out ${successCount} item(s) to ${stage?.name || 'stage'}.`, 'success')
    }
    if (failCount > 0) {
      showToast(`${failCount} item(s) failed — insufficient stock.`, 'error')
    }
    setCart([])
  ...
  const handleSaveItem = (id, form) => {
    if (id) {
      updateItem(id, form)
      showToast('Item updated.', 'success')
    } else {
      if (findItemByBarcode(form.barcode)) {
  ...
  const confirmAddStage = () => {
    ...
    if (stage) {
      showToast(`Stage "${stage.name}" added.`, 'success')
      refresh()
      setAddStageModal(false)
    } else {
      showToast('Stage name already exists.', 'error')
    }
  }

  const handleDeleteStage = (stageId) => {
    setDeleteStageConfirm(stageId)
  }
  ...
  const handleExportExcel = () => {
    try {
      ...
      const rows = itemsList.map(item => [
        ...
        item.total, Math.max(0, item.remaining),
  ...
  const itemsWithBreakdown = items.map(item => ({
    ...item,
    stageBreakdown: stages.reduce((acc, s) => {
  ...
  const transactionLedger = getLedgerWithDetails()

  const handleReverseTransaction = (entryId, note = '') => {
    const ok = reverseTransaction(entryId, note)
    if (ok) {
      showToast('Transaction reversed.', 'success')
      refresh()
    } else {
      showToast('Cannot reverse — insufficient stock.', 'error')
    }
  }

  const totalConsumed = stages.reduce((acc, s) => {
    ...
    <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <div className="flex items-center gap-1 px-4 py-1.5 border-b border-border bg-card">
        {[
          ...
          { id: 'stages', label: 'Stages', icon: Tent },
          ...
          <aside className="w-72 flex-shrink-0 flex flex-col gap-3 p-3 overflow-y-auto border-r border-border bg-card/50">
            ...
            <div className="bg-card border border-border rounded-lg p-3">
              ...
              <form onSubmit={handleBarcodeSubmit}>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-background border border-input rounded-lg py-2.5 pl-9 pr-3 text-sm text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                  />
                </div>
              </form>
              ...
              <button
                ...
                className={`mt-2 w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  importMode
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                  importMode ? 'bg-blue-500 text-white' : 'bg-muted text-muted-foreground'
                }`}>
                  {importMode ? 'ON' : 'OFF'}
                </span>
              </button>
              {importMode && (
                <p className="text-xs text-blue-500 mt-1.5">
                  Scanning adds items to stock. Unknown barcodes open the add-item form.
                </p>
              )}
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    ...
                    {stages.map(s => (
                      <th key={s.id} className="text-center py-3 px-3 font-semibold text-muted-foreground whitespace-nowrap">{s.name}</th>
                    ))}
                    ...
                    <tr key={item.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                      ...
                      {stages.map(s => (
                        <td key={s.id} className="text-center py-2.5 px-3">
                          {item.stageBreakdown[s.id] > 0 ? (
                            <span className="font-semibold text-primary">{item.stageBreakdown[s.id]}</span>
                          ) : (
                      ...
                      <td className="text-center py-2.5 px-3">
                        <StockBadge remaining={item.remaining} min={item.minStockThreshold || 10} />
                      </td>
                    </tr>
                  ))}
                  {items.length === 0 && (
                    <tr>
                      <td colSpan={stages.length + 3} className="text-center py-12 text-muted-foreground">
                        No items to display.
                      </td>
                    </tr>
                  )}
                ...
                <tfoot>
                  <tr className="border-t-2 border-border bg-muted/30">
                    ...
                    {stages.map(s => (
                      <td key={s.id} className="text-center py-3 px-3 font-bold text-destructive">
                        {totalConsumed[s.id] > 0 ? totalConsumed[s.id] : '—'}
                      </td>
                    ))}
                    <td colSpan={2} />
                  </tr>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl">
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="space-y-2">
                <button onClick={handleExportExcel} className="w-full flex items-center gap-2 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:opacity-90 transition-opacity">
                  <Download size={16} /> Export to Excel
                </button>
                <button onClick={handleExportJson} className="w-full flex items-center gap-2 px-4 py-2.5 rounded-lg bg-card border border-input text-foreground font-medium text-sm hover:bg-accent transition-colors">
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="space-y-2">
                <button onClick={handleImportJson} className="w-full flex items-center gap-2 px-4 py-2.5 rounded-lg bg-card border border-input text-foreground font-medium text-sm hover:bg-accent transition-colors">
                  <Upload size={16} /> Import from JSON
                </button>
                <button onClick={handleSeed} className="w-full flex items-center gap-2 px-4 py-2.5 rounded-lg bg-card border border-input text-foreground font-medium text-sm hover:bg-accent transition-colors">
                  <Plus size={16} /> Load Sample Data
                </button>
              </div>
            </div>
          </div>
      ...
      {returnItemData && (
        <ReturnQtyModal
          item={returnItemData.item}
          stageName={activeStage?.name || ''}
          maxReturn={returnItemData.maxQty}
      ...
      {addStageModal && (
        <ConfirmationModal
          ...
          confirmLabel="Add"
          cancelLabel="Cancel"
          onConfirm={confirmAddStage}
          onCancel={() => setAddStageModal(false)}
        >
          <input
            type="text"
            value={newStageName}
            ...
            autoFocus
            onKeyDown={e => { if (e.key === 'Enter') confirmAddStage() }}
          />
        </ConfirmationModal>
      )}
      ...
      {deleteStageConfirm && (
        <ConfirmationModal
          title="Delete Stage"
          message="Delete this stage? All items assigned will be returned to stock."
          ...
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {stages.map(stage => (
                <button
                  key={stage.id}
                  onClick={() => handleCheckoutStagePick(stage.id)}
            ...
            <button
              onClick={() => setCheckoutStagePicker(false)}
              className="w-full mt-3 px-4 py-2.5 rounded-lg border border-input text-foreground hover:bg-accent transition-colors font-medium text-sm"
            >
              Cancel
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-lg mx-4 shadow-2xl">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Checking out to <span className="font-semibold text-foreground">{stages.find(s => s.id === checkoutReview.stageId)?.name}</span>
            </p>
            ...
            <div className="max-h-60 overflow-y-auto space-y-1.5 mb-4">
              {checkoutReview.cart.map((entry, idx) => (
                <div key={idx} className="flex items-center justify-between bg-accent/30 rounded-lg px-3 py-2">
                  <div>
                    <div className="text-sm font-semibold text-foreground">{entry.item.name}</div>
                    <div className="text-xs text-muted-foreground font-mono">{entry.item.barcode}</div>
                  </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-lg border border-input text-foreground hover:bg-accent transition-colors font-medium text-sm"
              >
                Cancel
              </button>
              <button
                onClick={confirmCheckout}
                className="flex-1 px-4 py-2.5 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:opacity-90 transition-colors"
              >
                Confirm Checkout
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <p className="text-muted-foreground text-sm mb-1">
              Only <span className="font-semibold text-foreground">{addStockPrompt.item.remaining}</span> of <span className="font-semibold text-foreground">{addStockPrompt.item.name}</span> available.
            </p>
            <p className="text-muted-foreground text-sm mb-5">
              Add stock to increase quantity to {addStockPrompt.desiredQty}?
            </p>
            <div className="flex gap-3">
              <button
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-lg bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 transition-colors"
              >
                Add {addStockPrompt.desiredQty - addStockPrompt.currentQty} to Stock
              </button>
            </div>
          </div>
        </div>
      ...
      {unknownBarcode && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-xl p-6 w-full max-w-sm mx-4 shadow-2xl">
            ...
            <p className="text-muted-foreground text-sm mb-1">
              Barcode <span className="font-mono font-semibold text-foreground">{unknownBarcode}</span> was not found in inventory.
            </p>
            <p className="text-muted-foreground text-sm mb-5">
              Switch to Import Mode to add this item?
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => {
                  setImportMode(true)
                  setEditItem({ barcode: unknownBarcode, name: '', description: '', category: 'General', total: 1, unitQuantity: 1, unitType: 'pcs', minStockThreshold: 10 })
